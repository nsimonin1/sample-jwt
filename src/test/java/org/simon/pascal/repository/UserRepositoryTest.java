...
public class UserRepositoryTest extends BaseRepositoryTest {
	...
	public void findByIdTest() {
		final User user = getUsers(1, 1001).get(0);
	...
	public void findByEmailTest() {
		final User user = getUsers(1, 1001).get(0);
		final User fetchedUser = userRepository.findByEmail(user.getEmail());
		assertNotNull("User shouldn't be NULL", fetchedUser);
		assertEquals("Should return appropriate user", user, fetchedUser);
	}
	...
	public void findByNameTest() {
		final User user = getUsers(1, 1001).get(0);
		final User fetchedUser = userRepository.findByName(user.getName());
		assertNotNull("User shouldn't be NULL", fetchedUser);
		assertEquals("Should return appropriate user", user, fetchedUser);
	}
	...
	public void findAllTest() {
		final List<User> users = getUsers(3);
		final List<User> fetchedUsers = userRepository.findAll();
	...
	public void changeUsernameTest() {
		final User user = userRepository.findOne(1001L);
		user.setName("another random name");
		final User changedUser = userRepository.findOne(1001L);
